...
class UDADataset(object):
    ...
    def __init__(self, source, target, prompt, cfg):
        self.source = source
        self.target = target
        self.prompt = prompt
    ...
    def get_rare_class_sample(self):
        ...
        s2 = self.target[i2]
        i3 = np.random.choice(range(len(self.prompt)))
        s3 = self.prompt[i3]
        ...
        out = {
            **s1, 'target_img_metas': s2['img_metas'],
            'target_img': s2['img'],
            'prompt_img': s3['img'],
            'prompt_img_metas': s3['img_metas'],
            'prompt_semantic_seg': s3['gt_semantic_seg']
        }
